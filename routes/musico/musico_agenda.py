...
async def get_agenda_musico(request: Request, nome_usuario: str):
...
async def get_disponibilidade_musico(request: Request, nome_usuario: str):
    sessao_usuario = request.session.get("usuario")
    if not sessao_usuario or sessao_usuario.get("nome_usuario") != nome_usuario:
        return RedirectResponse(url="/login", status_code=303)
    ...
    usuario = usuario_repo.UsuarioRepo("dados.db").get_by_nome_usuario(nome_usuario)
    response = templates.TemplateResponse(
        "/musico/alterar_disponibilidade.html",
        {
            "request": request, 
            "nome_usuario": nome_usuario,
            "usuario": usuario 
